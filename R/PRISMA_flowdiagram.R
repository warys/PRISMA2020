...
PRISMA_flowdiagram <- function( #nolint
  ...
  #wrap exclusion reasons (skip if NA — no data to display)
  if (is.data.frame(dbr_excluded)) {
  ...
  if (is.data.frame(database_specific_results)) {
    database_specific_results[, 1] <- stringr::str_wrap(
      database_specific_results[, 1],
      width = 35
    )
  }
  if (is.data.frame(register_specific_results)) {
  ...
  if (other == TRUE) {
    ...
    other_identified_label <- paste0(
      ifelse(
        nchar(cond_websites) > 0 | nchar(cond_organisation) > 0 | nchar(cond_citation) > 0,
    ...
    other_excluded_label <- paste0(
      other_excluded_text,
      ifelse(nchar(other_excluded_data) > 0, ":", ""),
      other_excluded_data
    )
    # heights
  ...
  # When all removal reasons are NA, leave cond_duplicates empty
  # (no forced "(n = 0)" — box and header are suppressed automatically)
  ...
  # Only show "Records identified from:" header when there's actual data
  dbr_identified_label <- paste0(
    ifelse(
      nchar(cond_database) > 2 | nchar(cond_register) > 2,
      "Records identified from:",
      ""
    ),
    cond_database,
  ...
  dbr_excluded_label <- paste0(
    dbr_excluded_text,
    ifelse(nchar(dbr_excluded_data) > 0, ":", ""),
    dbr_excluded_data
  )
  ...
  # Only show "Records removed before screening:" when there's actual data
  dbr_notscreened_label <- paste0(
    ifelse(
      nchar(cond_duplicates) > 1 | nchar(cond_automatic) > 1 | nchar(cond_exclother) > 1,
